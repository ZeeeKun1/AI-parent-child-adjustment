...
浏览器媒体已经接入新的`runtime`会话调度层，模块一至四形成可运行闭环：滚动音视频窗口进入Qwen状态识别，随后经过轨迹控制、策略选择和输出协调，浏览器呈现文字、可选播放Maia语音并回传两个通道的执行结果；干预后的下一轮只分析新采集媒体并记录恢复状态。

默认`web-live`仍是零API采集模式。必须显式加`--enable-closed-loop`才调用状态识别，`--max-assessments`限制单会话调用次数；必须再加`--enable-voice`才调用TTS。原始PCM/JPEG和生成语音均不落盘。2026-08-05本地全量测试为79项通过，尚未进行真实摄像头、麦克风、真实Qwen闭环和服务器HTTPS/WSS验收，也尚未提交本轮改动。

当前下一步不是继续重写闭环，而是先审查本轮本地改动，再用很小的`--max-assessments`完成一次真实浏览器、双设备和Qwen端到端测试。下方“尚未接入Qwen”等描述是2026-08-03历史基线，不能再作为当前代码状态。

## 0. 新对话必须先做什么

请完整读取本文件，并优先遵循上方“最新实现状态”。Windows DirectShow、浏览器采集和本地闭环代码都已经完成，不要重新实现；下一任务是审查并用最小额度进行真实端到端验收。
