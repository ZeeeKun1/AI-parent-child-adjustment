...
浏览器录制的两段声音由服务器通过腾讯云官方 SDK 注册为本次会话的临时声纹；服务器不把原始录音或声纹 ID 写入磁盘。实验结束时，服务器先逐个删除云端家长与儿童声纹，再删除空分组；清理结果写入 `events.jsonl`。实验记录只包含派生状态、时间线、匹配分数、低置信标记和运行指标。若论文评估需要事后人工复核原始互动，应另行使用经过伦理审批的录制方案。
