...
服务器部署方向已经加入浏览器采集与可选闭环服务。参与者可打开同源网页，在明确授权后由浏览器采集摄像头和麦克风，并通过 WebSocket 向服务器发送相同格式的`MediaChunk`。默认命令仍只验证采集且不调用 Qwen；只有显式启用闭环后，滚动媒体窗口才会依次进入状态识别、介入时机、策略选择和浏览器干预。原始音视频始终不落盘。
