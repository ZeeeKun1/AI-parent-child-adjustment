...
当前仓库包含独立、可迁移的技术验证骨架和浏览器实验界面；它仍是研究原型，不是已经完成真实部署验收的正式实验系统。参与者音视频或API密钥不会纳入版本控制。
...
服务器部署方向已经加入浏览器采集与可选闭环服务。参与者可打开同源网页，在明确同意音视频留存后，由浏览器通过 WebSocket 发送实时识别所需的`MediaChunk`，并通过同源 HTTPS 分段保存一份完整活动录像。默认命令不调用 Qwen；只有显式启用闭环后，滚动媒体窗口才会依次进入状态识别、介入时机、策略选择和浏览器干预。
...
然后在共同观看的平板上打开`http://127.0.0.1:8000`，研究人员可在电脑打开`http://127.0.0.1:8000/research`。家庭端只呈现活动、必要提示与自主控制；研究端显示会话状态、事件时间线和专家接管面板，不在线传输或播放完整录像。录像保存在服务器对应的匿名实验运行目录中。

服务器部署时，家庭端无需邀请码：填写基本信息、选择角色并完成两段声音采集后即可开始。服务器通过腾讯云官方 SDK 临时注册双方声纹，互动中分别执行两次 1:1 比对以区分家长与儿童，结束后删除云端声纹。腾讯密钥仅配置在服务器环境变量中。研究控制台仍使用独立的`RESEARCH_CONSOLE_ACCESS_TOKEN`。公网必须通过 HTTPS/WSS 访问，且启动命令需显式加入`--enable-closed-loop`；完整步骤见[阿里云部署说明](docs/aliyun-deployment.md)。
...
浏览器将音频转换为16 kHz单声道PCM16、约100 ms一块，将视频约每秒压缩为一张最大1280×720且带`frame_time_ms`的JPEG，用于实时判断；这些推理片段不单独落盘。同时，浏览器每10秒上传一段MediaRecorder录像，服务器在同一匿名实验目录中合并保存。设备标识、远程IP、访问码和API密钥不会写入记录。详细说明见[浏览器服务器部署说明](docs/browser-deployment.md)。
...
每次运行都会在`data/output/runs/<run_id>/`生成独立证据目录，其中包含输入哈希、模型与编码表版本、实际提示词、收发事件、转录事件、本地声学测量、延迟指标和经 schema 校验的状态结果。浏览器正式实验还会在`media/`保存经同意录制的完整活动录像；API密钥不会写入日志。
